package functionalProgramming.chapter4.statethread
...
object STRef {
  ...
  def test: ST[Nothing, (Int, Int)] = for {
...
sealed abstract class STArray[S, A](implicit manifest: Manifest[A]) {
  ...
  def fill(xs: Map[Int, A]): ST[S, Unit] =
    xs.foldLeft(ST[S, Unit](())) { case (s, (k, v)) => s flatMap (_ => write(k, v)) }
...
sealed trait STHashMap[S,K,V] {
  ...
  def update(k: K, v: V): ST[S, Unit] = ST(table.update(k,v))

  def remove(k: K) = ST(table.remove(k))

  def get(k: K): ST[S, Option[V]] = ST(table.get(k))

  def freeze: ST[S, Map[K,V]] = ST(table.toMap)

  def apply(k: K): ST[S,V] = ST(table(k))
}

object STHashMap {
  def empty[S, K, V]: STHashMap[S,K,V] = new STHashMap[S,K,V] {
    lazy val table: mutable.HashMap[K, V] = mutable.HashMap.empty
  }

  def fromMap[S, K, V](m : Map[K,V]): STHashMap[S,K,V] = new STHashMap[S,K,V] {
    lazy val table: mutable.HashMap[K, V] = mutable.HashMap(m.toSeq: _ *)
  }
}
